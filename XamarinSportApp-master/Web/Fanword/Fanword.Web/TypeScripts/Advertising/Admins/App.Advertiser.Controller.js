(function () {
    ...
    function advertiserController($scope, $http, $state, $timeout) {
        resetLoading();
        ...
        function loadControllerData() {
            if ($state.current.name == "viewAdvertiser") {
                $scope.searchText = "";
                $http.get('/api/Advertisers/' + $state.params.id).then(function (promise) {
                    $scope.advertiser = promise.data;
        ...
        function saveAdvertiser() {
            $scope.modelState = {};
            $http.put('/api/Advertisers', $scope.advertiser).then(function (promsie) {
                $state.go('root');
...
//# sourceMappingURL=App.Advertiser.Controller.js.map