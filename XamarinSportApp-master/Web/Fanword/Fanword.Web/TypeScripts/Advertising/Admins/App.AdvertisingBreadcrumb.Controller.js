(function () {
    ...
    function advertisingBreadcrumbController($scope, $state, $http) {
        $scope.crumbs = [];
        ...
        if ($state.current.name == "viewCampaign") {
            defaultCrumbs();
            $http.get('/api/Advertisers/' + $state.params.advertiserId).then(function (promise) {
                $scope.crumbs.push(new Breadcrumb(promise.data.companyName, "/Advertising#!/View/" + promise.data.id));
...
//# sourceMappingURL=App.AdvertisingBreadcrumb.Controller.js.map