(() => {
    ...
    function contentManagementController($scope: any, $http: ng.IHttpService, $state: any) {
        $scope.searchText = "";
        function setupControls() {
            $scope.postOptions = {
                ...
                toolbar: [
                    {
                        template:
                        "<input type='text' class='k-textbox agilx-name-lg pull-left' ng-model='searchText' ng-change='onSearchChange()' style='width:300px'  placeholder='Search content or author' />" +
        ...
        $scope.deletePost = (kendoData: any) => {
            swal({
                title: 'Are you sure?',
